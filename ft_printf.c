/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_printf.c                                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: aialonso <aialonso@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/31 11:11:00 by aialonso          #+#    #+#             */
/*   Updated: 2025/11/01 17:38:23 by aialonso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int	get_type(va_list arg, int typ)
{
	if (typ == 'c')
		return (ft_putchar_fd((char)va_arg(arg, int), 1));
	if (typ == 's')
		return (ft_putstr_fd(va_arg(arg, char *), 1));
	if (typ == 'p')
		return (ft_ptoa(va_arg(arg, void *)));
	if (typ == 'd')
		return (ft_putstr_fd(ft_itoa(va_arg(arg, int)), 1));
	if (typ == 'i')
		return (ft_putstr_fd(ft_itoa(va_arg(arg, int)), 1));
	if (typ == 'u')
		return (ft_putstr_fd(ft_utoa(va_arg(arg, unsigned int)), 1));
	if (typ == 'x')
		return (ft_putstr_fd(ft_up_hetoa(va_arg(arg, unsigned int)), 1));
	if (typ == 'X')
		return (ft_putstr_fd(ft_lo_hetoa(va_arg(arg, unsigned int)), 1));
	if (typ == '%')
		return (ft_putchar_fd('%', 1));
	return (0);
}

int	ft_printf(const char *text, ...)
{
	va_list	arg;
	size_t	n;
	size_t	charcoun;

	n = 0;
	charcoun = 0;
	va_start(arg, text);
	while (text[n])
	{
		if (text[n] == '%')
		{
			charcoun = charcoun + get_type(arg, text[n + 1]);
			n = n + 2;
		}
		else
		{
			write(1, &text[n], 1);
			charcoun++;
			n++;
		}
	}
	return (charcoun);
}

/* int	main(void)
{
	printf("%d\n", ft_printf("%p\n", &"a"));
	printf("%d\n", printf("%p\n", &"a"));
	return (0);
} */
